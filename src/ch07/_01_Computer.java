...
public abstract class _01_Computer {
     ...
     *    재정의하지 않으면 컴파일 오류가 발생한다. (재정의 안 할 경우, 추상클래스로 존재해야함)
     *  - 자식클래스에게 강제성과 통일성을 줄 수 있다.(협업시 작업명세서)  
     *       
     * 4. 추상클래스의 객체 생성
     * - 추상클래스는 객체를 생성할 수 없으며(추상 메서드 때문), 상속 계층에서
     *   추상적인 개념을 나타내기 위한 용도로 사용한다.
     * - 상속을 통하여 자식클래스에서 추상메서드를 구현하고, 자식클래스 객체를 생성하여 메서드에 접근할 수 있다.  
     * 
     */
	...
	// 추상메서드이므로 클래스도 추상클래스가 됨!
	// 추상메서드가 하나라도 있으면 클래스도 추상클래스가 되어야 함.
